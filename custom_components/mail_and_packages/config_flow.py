...
def _normalize_shopify_senders(user_input: dict) -> dict:
    """Resolve a submitted Shopify sender scope, treating empty as cleared.

    The form offers the stored scope as a suggested value, so clearing the
    field submits nothing at all for it. Both that and the "(none)" sentinel
    mean "no scope" and must overwrite the stored list — otherwise the step
    leaves the previous value in place and the clear is silently undone.
    """
    value = user_input.get(CONF_SHOPIFY_SENDERS)
...
def _get_schema_step_shopify(
    ...
    """Get a schema pre-filled from default_dict.

    The stored scope is offered as a `suggested_value` rather than a schema
    `default`. A default would be re-injected by voluptuous whenever the
    field is submitted empty, silently restoring the very value the user
    just deleted; a suggested value pre-fills the box but leaves the key
    absent when it is cleared, which the step treats as "no scope".
    """
    if user_input is None:
        user_input = {}

    def _get_suggested(key: str, fallback: Any = None) -> Any:
        """Get the pre-filled value for key."""
        value = user_input.get(key, default_dict.get(key, fallback))
    ...
    return vol.Schema(
        {
            vol.Optional(
                CONF_SHOPIFY_SENDERS,
                description={
                    "suggested_value": _get_suggested(
...
class MailAndPackagesFlowHandler(
    ...
    async def async_step_config_shopify(self, user_input=None):
        ...
        self._errors = {}
        if user_input is not None:
            user_input = _normalize_shopify_senders(user_input)
            self._data.update(user_input)
            self._errors, user_input = await _validate_user_input(self._data, self.hass)
            if len(self._errors) == 0:
                if (
